document.addEventListener("DOMContentLoaded", function () {
  ...
  var buildQuiz = async function (quizNode) {
    ...
    var errorText = quizNode.getAttribute(
      "data-" + LightningComments + "-error"
    );
    var questions = parseQuiz(
      quizNode.getAttribute("data-" + LightningComments)
    );
    var correctId =
      LightningComments +
      location.pathname +
    ...
    var errorNode = document
      .createElement("h3")
      .appendChild(document.createTextNode(errorText)).parentNode;
    ...
    questions.forEach(function (question, index) {
      ...
      question.answer
        .map(function (key, val) {
          return key && buildAnswer(key, LightningComments + index, val);
        })
        .sort(function () {
    ...
    quizNode.addEventListener("change", function () {
      var checked = questions.map(function (q, i) {
        return container.querySelector(
          'input[name="' + LightningComments + i + '"]:checked'
        );
      });
      ...
      var failure =
        !correct && checked.filter(Boolean).length === questions.length;
    ...
    const invoiceAmount = document.getElementsByName(
      "lncomments_settings[lncomments_text_field_3]"
    )[0].value;
    const lnbitsUrl = document.getElementsByName(
      "lncomments_settings[lncomments_text_field_1]"
    )[0].value;
    const lnbitsKey = document.getElementsByName(
      "lncomments_settings[lncomments_text_field_2]"
    )[0].value;
    const btcpayserverUrl = document.getElementsByName(
      "lncomments_settings[lncomments_text_field_4]"
    )[0].value;
    const btcpayserverStoreId = document.getElementsByName(
      "lncomments_settings[lncomments_text_field_5]"
    )[0].value;
    const btcpayserverToken = document.getElementsByName(
      "lncomments_settings[lncomments_text_field_6]"
    )[0].value;
    ...
    try {
      if (lnbitsUrl && lnbitsKey) {
        ...
        const invoiceResponse = await fetch(`${lnbitsUrl}/api/v1/payments`, {
          ...
          body: JSON.stringify({
            ...
            memo: "Pay to comment on " + window.location.host,
          }),
        });
        ...
        if (invoice?.payment_request) {
          ...
          const interval = setInterval(async () => {
            const paidResponse = await fetch(
              `${lnbitsUrl}/api/v1/payments/${invoice.payment_hash}`,
              {
                headers: {
                  Accept: "application/json",
                  "Content-Type": "application/json",
        ...
        const invoiceResponse = await fetch(
          `${btcpayserverUrl}/api/v1/stores/${btcpayserverStoreId}/invoices`,
          {
            headers: {
              Accept: "application/json",
              "Content-Type": "application/json",
        ...
        const actualInvoiceResponse = await fetch(
          `${btcpayserverUrl}/api/v1/stores/${btcpayserverStoreId}/invoices/${invoice.id}/payment-methods`,
          {
            headers: {
              Accept: "application/json",
              "Content-Type": "application/json",
        ...
        if (actualInvoice) {
          ...
          const interval = setInterval(async () => {
            const paidResponse = await fetch(
              `${btcpayserverUrl}/api/v1/stores/${btcpayserverStoreId}/invoices/${invoice.id}`,
              {
                headers: {
                  Accept: "application/json",
                  "Content-Type": "application/json",
      ...
      qrcode.style.background = "linear-gradient(135deg, orange, purple)";
      qrcode.style.padding = "10px";
      qrcode.style.borderRadius = "10px"; /* Optional: adds rounded corners */
      qrcode.width = 300; // Set the width of the canvas
      qrcode.height = 300; // Set the height of the canvas
      ...
      new QRious({
        ...
        size: 300, // Match size to fill the canvas
  ...
  [].forEach.call(
    document.querySelectorAll("." + LightningComments),
    buildQuiz
  );
